...
async function getDiscoverableTeacherOrNull(teacherUserId: string) {
  ...
  const calendarAvailable = teacher.teacherAvailabilityCalendar[0]?.isAvailable;
  const weeklyAvailable = teacher.teacherWeeklyAvailability[0]?.isAvailable ?? false;
  const isAvailableToday = calendarAvailable ?? weeklyAvailable;
